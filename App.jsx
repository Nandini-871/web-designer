...
<body class="p-[10px]">
    <h1 class="text-[30px] font-[700] text-violet-500 ">Welcome to Web Designer</h1>
</body>
</html>
    `
  );
  ...
  const downloadCode = () => {
    let filename = "webDesignerCode.html";
...
Final instruction: Output only the single fenced Markdown code block with the full HTML file content. Nothing else.
 ...
 Website prompt: ${prompt} `
  ...
  return (
    <>
     <div className= " min-h-screen bg-white dark:bg-black text-white dark:text-white transition-colors duration-300">
      <Navbar dark={dark} setDark={setDark} />
   
      
      <div className="container">
        <h3 className='text-[30px] font-[700] text-slate-600'>Create a beautiful website with 
          <span className='bg-gradient-to-bl from-slate-800 via-violet-500 to-zinc-400 bg-clip-text text-transparent'>
            Web Designer</span> </h3>
        <p className='mt-4 text-[16px] text-[#b3b3b3]'>Describe your website and ai will code for you</p>
        <div className="inputbox">
          <textarea onChange={(e) => { setPrompt(e.target.value) }} value={prompt} placeholder='describe your website in detail'></textarea>
          {
            prompt !== "" ? <>
              <i onClick={getResponse} className='sendIcon text-[20px] w-[30px] h-[30px] flex items-center justify-center bg-[#9933ff] rounded-[50%] cursor-pointer
            transition-all duration-300 hover:opacity-[.8]'><MdOutlineArrowUpward /></i>
            </> : ""
          }
        </div>  
   

        <p className='text-[20px] font-[700] mt-[5vh] bg-gradient-to-b
from-violet-400 via-violet-800 to-indigo-300 bg-clip-text text-transparent'> Your AI-generated website will appear here.</p>
        <div className="preview">
          <div className="header w-full h-[70px]">
            <h3 className='font-bold mt-5 text-[16px] '>Live Preview</h3>
            <div className="icons flex items-center gap-[15px]">
              ...
              <div onClick={() => { setIsShowCode(!isShowCode) }} className="icon !w-[auto] !p-[12px] flex items-center gap-[10px]">

                {isShowCode ? "Hide Code" : "Show Code"} {isShowCode ? <FaEyeSlash /> : <BiSolidShow />}</div>
            </div>
          </div>

          {
            isShowCode ? <>
              ...
              {
                loadind ? <div className='w-full h-full flex items-center justify-center flex-col' >
                  <FadeLoader color='#9933ff' />
                  <h3 className='text-[23px] mt-4 font-semibold' ><span className='bg-gradient-to-bl from-slate-800 via-violet-500 to-zinc-400 bg-clip-text text-transparent'>Generating</span> your website...</h3>
                </div> :
                  <>   <iframe srcDoc={code} className='w-full  bg-black dark:bg-white ' ></iframe>
                  </>}
            </>
          }
          ...
          <>
            <div className="modelcon">
              <div className="modelbox  ">
                ...
                <iframe srcDoc={code} className='newTabiframe w-full bg-white dark:bg-[#111]'></iframe>
              </div>
            </div>
          </> : ""
      }
